...
export default function QuestionCard({ question, renderTags }: Props) {
  return (
    <li
      id={question.id}
      className="study-question scroll-mt-[88px] py-4 first:pt-0"
    >
      <div className="flex items-start gap-1">
        <details className="group min-w-0 flex-1" suppressHydrationWarning>
          <summary className="-mx-2 -my-1 flex cursor-pointer list-none items-start justify-between gap-3 rounded-md px-2 py-1 select-none hover:bg-rule/40 [&::-webkit-details-marker]:hidden">
            ...
            <svg
              ...
              className="mt-1.5 h-4.5 w-4.5 shrink-0 text-muted transition-transform group-open:rotate-180 print:hidden"
            >
        ...
        <CopyQuestionLink questionId={question.id} />
      </div>
    </li>
  );
}
