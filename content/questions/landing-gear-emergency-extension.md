---
tags:
  - landing-gear
  - complex
---

### Question

How do you get the gear down if the normal system fails?

### Answer

Every retractable has an **emergency extension** method, and the type varies:

- **Hydraulic** systems typically have a hand pump, or a valve that dumps pressure and lets the gear **free-fall** into place, sometimes assisted by airspeed and gravity.
- **Electric** systems typically have a hand crank, and require **pulling the gear motor circuit breaker** first so the motor doesn't fight you.

Common points worth knowing for your airplane:

- Most emergency extensions are **one-way** - once you've used it, you cannot retract the gear in flight.
- Slowing down often helps the gear fall and lock.
- Confirm **three green** and check the mechanical indicator if fitted.

Know the specific procedure and the airspeed limits from the AFM, and know where the crank or handle physically is.

### Sources

- [FAA Airplane Flying Handbook, Chapter 11 - Transition to Complex Airplanes](https://www.faa.gov/sites/faa.gov/files/regulations_policies/handbooks_manuals/aviation/airplane_handbook/12_afh_ch11.pdf)
- [FAA Pilot's Handbook of Aeronautical Knowledge, Chapter 7 - Aircraft Systems](https://www.faa.gov/sites/faa.gov/files/09_phak_ch7.pdf)
