...
- **FIKI** (flight into known icing) approval means the airplane was **certified** with a complete, tested ice protection package, and the AFM/POH authorizes flight into known icing. It is an airplane-level certification, not a collection of parts.
- Having boots does **not** by itself make an airplane FIKI. Plenty of airplanes carry boots or a heated windshield without the approval.
- In a **non-FIKI** airplane, encountering icing is an emergency-in-the-making: the only correct plan is to exit the conditions - change altitude, reverse course, or land.
- Even in a FIKI airplane, the approval is a tool for **leaving** icing conditions safely, not a license to loiter in them. No airplane is approved for severe icing.
- Ice on the airframe raises stall speed, adds drag and weight, and degrades climb performance, so the airplane's margins shrink exactly when you need them.
