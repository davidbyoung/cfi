---
tags:
  - engine-failure
  - adm
  - flight-planning
---

### Question

What goes into a light twin takeoff briefing?

### Answer

The point is to make every decision **before** you need it. A workable structure:

- **Runway and performance** - Runway length available, accelerate-stop distance, and whether single-engine climb is positive at today's weight, altitude, and temperature.
- **Speeds** - Rotation speed, Vmc (red line), Vyse (blue line), Vxse if obstacles matter.
- **The decision point** - The specific condition that separates abort from continue. For example: any failure **on the runway**, abort; after liftoff with runway remaining and gear down, **land**; after liftoff, clean, and climbing at blue line, **continue**.
- **The plan if you continue** - Initial heading, altitude, obstacles or terrain, which airport you'll return to.
- **The plan if you can't climb** - Where you'll put it.

Say it out loud. The purpose is to convert a three-second reaction into the execution of a decision you already made.

### Sources

- [FAA Airplane Flying Handbook, Chapter 13 - Transition to Multiengine Airplanes](https://www.faa.gov/sites/faa.gov/files/regulations_policies/handbooks_manuals/aviation/airplane_handbook/14_afh_ch13.pdf)
- [FAA Commercial Pilot - Airplane ACS (FAA-S-ACS-7), Area of Operation X - Multiengine Operations](https://www.faa.gov/training_testing/testing/acs/commercial_airplane_acs_7.pdf)

### Supplements

- [AOPA - Multiengine Training](https://www.aopa.org/training-and-safety/learn-to-fly/flying-for-a-career/multiengine-training)
